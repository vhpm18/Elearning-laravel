    ...
    <div>
        ...
        <v-server-table ref="table" :columns="columns" :url="url" :options="options" >
            ...
            <div slot="created_at" slot-scope="props">
                {{ formattedDate(props.row.user.created_at) }}
            </div>
            <template slot="picture" slot-scope="props">
                <img :src="formattedImg(props.row.user.picture)" alt="" width="50" height="50" class="img-thumbnail">
            </template>
            <div slot="message" slot-scope="props">
                <i :data-id="props.row.user.id" class="btn btn-primary btnEmail fa fa-envelope-square" id="show-modal" v-on:click="loadDataUser(props.row.user)"></i>
            </div>
        </v-server-table>
    </div>
</template>
